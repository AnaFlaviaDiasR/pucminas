...
/**
 *TP05Q04-> Ordenacao Selecao Recursivo
...
*/
   ...
   /**
    * Organiza o array com o algoritmo de ordenacao recursivamente
    */
   public void selecao(){
     int menor= 0;
     recursivo(menor, 0);
   }
   //metodo que realiza o for do i
   public void recursivo(int menor, int i){
     if(i < n-1){
       menor= procurar(i+1, i);
       swap(menor, i);
       recursivo(menor, i+1);
     }
   }
   //metodo que realiza o for do j
   public int procurar(int j, int menor){
     if(j < n){
       if((array[menor].getPaisDeOrigem().compareTo(array[j].getPaisDeOrigem())) == 0){
         if ((array[menor].getNome().compareTo(array[j].getNome())) > 0){
           menor= j;
         }
       }else if((array[menor].getPaisDeOrigem().compareTo(array[j].getPaisDeOrigem())) > 0){
           menor = j;
       }
       menor= procurar(j+1, menor);
     }
     return menor;
   }
