...
*/
   ...
   /**
    * Metodo para organizar o array com o algoritmo Sheelsort
    */
    public void shellsort()throws Exception{
      ...
      do{
        h /= 3;
        for(int divisao= 0; divisao < h; divisao++){
          insercaoPorDivisoes(divisao, h);
        }
      }while (h != 1);
    }//fim Sheelsort
    ...
    * @throws Exception
    */
    public void insercaoPorDivisoes(int divisao, int h)throws Exception{
      for (int i= (h + divisao); i < n; i+=h){
        Series aux = array[i];
        int j = i - h;

        while( (j >= 0) && (array[j].getIdiomaOriginal().compareTo(aux.getIdiomaOriginal()) > 0) ){
          array[j + h]= array[j];
          j-=h;
        }
        while( (j >= 0) && (array[j].getIdiomaOriginal().compareTo(aux.getIdiomaOriginal()) == 0) && (array[j].getNome().compareTo(aux.getNome()) > 0) ){
          array[j + h]= array[j];
          j-=h;
        }
        array[j + h]= aux;
      }
    }//fim insercaoPorDivisoes
}
//-----------------------------------------------------------------------------------------------------

class TP05Q06OrdenacaoShellsort{
