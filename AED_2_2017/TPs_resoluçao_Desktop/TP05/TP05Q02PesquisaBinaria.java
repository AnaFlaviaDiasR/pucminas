...
*/
   ...
   /**
    * Pesquisa se existe uma serie dentro da lista
    * @param palavra String
    * @return resp boolean
    */
   public boolean pesquisaBinaria(String palavra){
     boolean resp= false;
     int meio;
     int dir = n-1;
     int esq = 0;
     ...
     while(esq <= dir){
       meio= (dir + esq)/2;
       if (palavra.equals(array[meio].getNome())){
         resp= true;
         esq= n;
       }
       else if(palavra.compareTo(array[meio].getNome()) > 0 ){
           esq= meio + 1;
       }
       else{
           dir= meio - 1;
...
class TP05Q02PesquisaBinaria{
    ...
    Lista lista= new Lista(1000);//referente a classe Lista
    ...
    //Verifica se a serie desejada esta contida na lista
    for (String i=MyIO.readLine(); i.equals("FIM") == false; i=MyIO.readLine()){
			if(lista.pesquisaBinaria(i)){
				MyIO.println("SIM");
			}else{
				MyIO.println("NÃO");
			}
