...
class Lista {
   ...
   /**
    * Metodo para ordenar as series utilizando o algoritmo de ordenacao Bolha.
    */
   public void bolha() {
     for (int i = (n - 1); i > 0; i--) {
       for (int j = 0; j < i; j++) {
         if (array[j].getNumeroDeTemporadas() > array[j + 1].getNumeroDeTemporadas()){
                 swap(j, j+1);
         }
         if(array[j].getNumeroDeTemporadas() == array[j + 1].getNumeroDeTemporadas()){
