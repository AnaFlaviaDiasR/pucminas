...
*/
   ...
   /**
   * Metodo para organizar o arranjo com o algoritmo de insercao
   */
   public void insercao() {
       for (int i = 1; i < n; i++) {
         Series tmp = array[i];
         int j = i - 1;
         while((j >= 0) && (array[j].getIdiomaOriginal().compareTo(tmp.getIdiomaOriginal()) > 0)){
           array[j + 1] = array[j];

           j--;
         }
         while((j >= 0) && (array[j].getIdiomaOriginal().compareTo(tmp.getIdiomaOriginal()) == 0) && (array[j].getNome().compareTo(tmp.getNome()) > 0)){
           array[j + 1] = array[j];
...
class TP05Q05OrdenacaoInsercao{
